...
CREATE TABLE t_organ (
  ...
  organ_code VARCHAR(50) COMMENT '机构编号' UNIQUE,
  organ_name VARCHAR(200) COMMENT '机构名称' DEFAULT '未定义',
  parent_id  INT COMMENT '上级机构标识'
)
  COMMENT '组织机构表';
...
CREATE TABLE t_user (
  ...
  user_name VARCHAR(50) COMMENT '用户名' UNIQUE,
  user_password VARCHAR(50) COMMENT '密码' NOT NULL,
  organ_id INT COMMENT '所属机构标识',
  role_id  INT COMMENT '所属角色'
)
  COMMENT '用户表';
...
CREATE TABLE t_role (
  ...
  role_name VARCHAR(100) COMMENT '角色名称' NOT NULL
)
  COMMENT '角色表';
-- t_privilege
CREATE TABLE t_privilege (
  privilege_id   INT COMMENT '权限标识' AUTO_INCREMENT PRIMARY KEY,
  privilege_name VARCHAR(100) COMMENT '权限名称' NOT NULL
)
  COMMENT '权限表';
-- t_role_privilege
